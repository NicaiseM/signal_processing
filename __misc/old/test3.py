...
def find_peaks(time, *ch, distance, height, prominence):
    for i in ch:
        peak_indexes, peak_heights = fp(i, distance=distance, height=height,
                                        prominence=prominence)
        peak_time = np.array([time[i] for i in peak_indexes])
        peak_heights = peak_heights['peak_heights']
    return (peak_indexes, peak_time, peak_heights)


# def find_second_peaks(peak_times, t_expand_left, t_expand_ right, *ch):
#     for i in peak_times:
#         t, ch1, ch2 


def csv_processing(file):
    device, t, ch1, ch2 = read(file)
    settings = {}  # Вывести в функцию
    settings_tmp = ppc['common'].copy()
    settings_tmp.update(ppc[device])
    ...
    if settings['time_shift']:
        t = time_shift_removal(t)
    if settings['channel_shift']:
        ch1 = channel_shift_removal(ch1, settings['zero_amount'])
        if ch2 is not None:
            ch2 = channel_shift_removal(ch2, settings['zero_amount'])
    if not settings['t_left']:
        settings['t_left'] = t[0]
    if not settings['t_right']:
        settings['t_right'] = t[-1]
    if ch2 is None:
        t, ch1 = time_limitation(t, settings['t_left'],
                                 settings['t_right'], ch1)
    else:
        t, ch1, ch2 = time_limitation(t, settings['t_left'],
                                      settings['t_right'], ch1, ch2)
    if settings['smooth']:
        ch1 = smooth(ch1, settings['smooth_window'])
        if ch2 is not None:
            ch2 = smooth(ch2, settings['smooth_window'])
    if settings['find_peaks']:
        ch1_peaks = find_peaks(t, ch1, distance=0.003*len(ch1),
                               height=0.1*max(ch1), prominence=0.08*max(ch1))
        if ch2 is not None:
            ch2_peaks = find_peaks(t, ch2, distance=0.003*len(ch2),
                                   height=0.1*max(ch2),
                                   prominence=0.08*max(ch2))
        for i in ch1_peaks[1]:
            

    plt.clf()
    plt.plot(t*settings['t_factor'], ch1*settings['ch_factor'],
             'g-', label='u(t)')
    plt.plot(ch1_peaks[1]*settings['t_factor'], ch1_peaks[2]*settings['ch_factor'],
             'bx')
    plt.grid(True)
    ...
    if settings['show_title']:
        plt.title('Мощность в канале разряда для "{}"'.format(x))
    # plt.show()
    plt.savefig(ppltout, format=fig_format, dpi=fig_dpi)
    print('end')
